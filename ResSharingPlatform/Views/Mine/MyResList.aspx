...
<table class="wenkuTable">
    <% 
        foreach (var item in ViewData["reslist"] as List<ResSharingPlatform.Models.AppendixByLike_Result>)
        {
            string fileName = "";
            string ext = "";
        ...
        <tr>
            <td>
                ...
                <div class="wenkuEnd">
                    ...
                    | 贡献者：<%:item.USER_NAME %>
                    | 有效期：<%:Convert.ToDateTime(item.ACTIVE_TIME_START == null ? "1900/01/01" : item.ACTIVE_TIME_START.ToString()).ToString("yyyy/MM/dd")%>&nbsp;&nbsp;~&nbsp;&nbsp;
                              <%:Convert.ToDateTime(item.ACTIVE_TIME_END == null ? "1900/01/01" : item.ACTIVE_TIME_END.ToString()).ToString("yyyy/MM/dd")%>
