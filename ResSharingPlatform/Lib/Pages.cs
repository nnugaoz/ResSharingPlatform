    ...
    public class Pages
    {
        ...
        private string CreatePages()
        {
            ...
            if (subnums > subeachnums)
            {
                ...
                // 添加页面跳转
                subpagecss = subpagecss + "  第<input type=\"text\" id=\"pagehow\" style=\"width:30px;\" />页<input type=\"button\" value=\"跳转\" onclick=gotopage('" + subpagenums + "') /> ";
        ...
        private string CreateFormAction()
        {
            ...
            // 翻页  
            sb.Append("<script>function page(page){$(\"#pagecurrent\").val(page);$(\"#hpagecurrent\").val(page);GetInitList();}</script>");
            // 分页跳转
            sb.Append("<script>function gotopage(allpage){");
            // 判断输入跳转页是否为数字
            sb.Append("if($.trim(($(\"#pagehow\").val()))==\"\"){alert(\"请输入数字！\");return false;}");
            sb.Append("if(isNaN($(\"#pagehow\").val())) {alert(\"请输入数字！\");return false;}");
            sb.Append("if(parseInt($(\"#pagehow\").val()) > allpage || parseInt($(\"#pagehow\").val())<=0){alert(\"页索引超出范围！\");return false;}");
